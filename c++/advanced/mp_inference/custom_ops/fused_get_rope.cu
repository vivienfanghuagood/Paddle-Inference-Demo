...
__global__ __launch_bounds__(kBlockSize) void fused_get_rotary_embedding(const int64_t* position_ids, 
    ...
    */
    ...
    for(int idx = global_thread_idx, step=blockDim.x * gridDim.x; idx < elem_cnt; idx += step){
        ...
        const int64_t position_offset = bsz_idx * max_position_seq_length + seq_idx;
...
std::vector<paddle::Tensor> GetRoPE(const paddle::Tensor& input_ids, const paddle::Tensor& position_ids, const paddle::Tensor& head_dim_shape_tensor) {
