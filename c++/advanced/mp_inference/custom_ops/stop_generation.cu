...
__global__ void set_value_by_flags(const bool *stop_flags, int64_t *topk_ids, bool *stop_flags_out, const int bs, const int64_t end_id) {
    int tid = threadIdx.x;
    if (tid < bs) {
...
__global__ void set_value_by_flags_both(const bool *flags, const bool *stop_flags, int64_t *topk_ids, bool *stop_flags_out, const int bs, const int64_t end_id) {
    int tid = threadIdx.x;
    if (tid < bs) {
...
std::vector<paddle::Tensor> GetStopFlags(const paddle::Tensor& topk_ids, const paddle::Tensor& stop_flags, int64_t end_id, int64_t mode) {
    ...
    if (mode == 0 || mode == 1) {
        constexpr char *path = "/root/paddlejob/workspace/env_run/lzy/ERNIE_ALL/ERNIE3.0-fused-fp16/ops/test";
        ...
        if (mode == 0) {
            set_value_by_flags_both<<<1, block_size, 0, cu_stream>>>(flags_gpu.data<bool>(), stop_flags.data<bool>(), topk_ids_out.data<int64_t>(), stop_flags_out.data<bool>(), bs_now, end_id);
        } else {
            set_value_by_flags<<<1, block_size, 0, cu_stream>>>(flags_gpu.data<bool>(), topk_ids_out.data<int64_t>(), stop_flags_out.data<bool>(), bs_now, end_id);
        }
    } else if (mode == 2) {
        int block_size = (bs_now + 32 - 1) / 32 * 32;
        set_value_by_flags<<<1, block_size, 0, cu_stream>>>(stop_flags.data<bool>(), topk_ids_out.data<int64_t>(), stop_flags_out.data<bool>(), bs_now, end_id);
    } 
    return {topk_ids_out, stop_flags_out};
}

std::vector<std::vector<int64_t>> GetStopFlagsInferShape(const std::vector<int64_t>& topk_ids_shape, const std::vector<int64_t>& stop_flags_shape) {
    return {topk_ids_shape, stop_flags_shape};
}

std::vector<paddle::DataType> GetStopFlagsInferDtype(const paddle::DataType& topk_ids_dtype, const paddle::DataType& stop_flags_dtype) {
    return {topk_ids_dtype, stop_flags_dtype};
}

PD_BUILD_OP(set_stop_value)
