...
template <paddle::DataType D>
std::vector<paddle::Tensor> top_p_sampling_kernel(const paddle::Tensor& x, const paddle::Tensor& top_ps) {
    ...
    auto topp_probs = paddle::full({bs, 1}, 0, x.dtype(), x.place());
    auto inds_input = paddle::full({bs, vocab_size}, -1, paddle::DataType::INT64, x.place());
    auto sorted_out = paddle::full({bs, vocab_size}, 0, x.dtype(), x.place());
    ...
    count = 1;
    // auto curand_states_buf = paddle::full({bs * static_cast<int64_t>(sizeof(curandState_t))}, 
    //                                       0, paddle::DataType::UINT8, x.place());
    // curandState_t* dev_curand_states = reinterpret_cast<curandState_t*>(curand_states_buf.data<uint8_t>());
...
std::vector<paddle::Tensor> TopPSampling(const paddle::Tensor& x, const paddle::Tensor& top_ps) {
    switch (x.type()) {
